...
namespace Graffiti.Core
{
    ...
    public class SearchIndex
    {
        ...
        private static  Document CreateDocument(Post t)
        {
            ...
            doc.Add(Field.Keyword("postid",t.Id.ToString()));
            doc.Add(Field.Text("title", t.Title));
            doc.Add(Field.Text("body", RemoveHtml(t.Body, 0)));
            doc.Add(Field.UnStored("exact", RemoveHtml(t.Body, 0), true));
            doc.Add(Field.UnIndexed("rawbody", t.Body));
            doc.Add(Field.Keyword("category", t.Category.Name));
            doc.Add(Field.Keyword("categoryid", t.Category.Id.ToString()));
            doc.Add(Field.Keyword("username", t.UserName));
            doc.Add(Field.Keyword("modifiedby", t.ModifiedBy));
            doc.Add(Field.Keyword("createdby", t.CreatedBy));
            foreach (string tag in Util.ConvertStringToList(t.TagList))
                doc.Add(Field.Text("tag", tag));
            doc.Add(Field.Keyword("author", t.CreatedBy));
            doc.Add(Field.Keyword("date", DateTools.DateToString(t.Published, DateTools.Resolution.HOUR)));
            doc.Add(Field.Keyword("createddate", DateTools.DateToString(t.CreatedOn, DateTools.Resolution.HOUR)));
            doc.Add(Field.Keyword("modifieddate", DateTools.DateToString(t.ModifiedOn, DateTools.Resolution.HOUR)));
            doc.Add(Field.Keyword("name", t.Name));
            doc.Add(Field.Keyword("image", t.ImageUrl ?? string.Empty));
            doc.Add(Field.Keyword("commentcount", t.CommentCount.ToString()));
            doc.Add(Field.Keyword("pendingcommentcount", t.PendingCommentCount.ToString()));
            doc.Add(Field.UnIndexed("propertykeys", t.PropertyKeys));
            doc.Add(Field.UnIndexed("propertyvalues", t.PropertyValues));
            return doc;
        }
        ...
        void BuildIndex()
        {
            ...
            try
            {
                ...
                {
                    rd = new RAMDirectory();
                    writer = new IndexWriter(rd, GetAnalyzer(), true);

                    //These are probably not needed for this index
            ...
            try
            {
                ...
                if (docId > -1)
                {
                    ...
                    for (int j = 0; j < tfv.Size(); j++)
                    {
                        ...
                        booleanQuery.Add(tq, false, false);
                    }
