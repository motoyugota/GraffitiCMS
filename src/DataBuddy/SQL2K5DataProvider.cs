...
namespace DataBuddy
{
	public class SQL2K5DataProvider : SQLDataProvider
	{
		...
		protected override string SelectPagedSql(string columnList, string tableName, string where, string order, string pk,
		                                         int pageIndex, int pageSize)
		{
			...
			sb.AppendFormat("SELECT * FROM __PagedTable where __ROW_INDEX > {0} AND __ROW_INDEX <= {1} ORDER BY __ROW_INDEX",
