...
public partial class graffiti_admin_posts_write_Default : ControlPanelPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if(!IsPostBack)
        {
            ...
            if(post != null)
            {
                ...
                if (post.Id > 0)
                {
                    ...
                    txtContent.Value = post.PostBody;
					txtContent_extend.Value = post.ExtendedBody;
        ...
        if (FormWrapper.Visible)
        {
            ...
            Page.ClientScript.RegisterStartupScript(GetType(), 
                "Writer-Page-StartUp", 
                "$(document).ready(function() { var eBody = $('#extended_body')[0]; " + (!string.IsNullOrEmpty(txtContent_extend.Value.Trim()) ? "eBody.style.position = 'static'; eBody.style.visibility = 'visible';" : "eBody.style.position = 'absolute'; eBody.style.visibility = 'hidden';") + "categoryChanged($('#" + CategoryList.ClientID + "')[0]); Publish_Status_Change();});", true);
    ...
    protected void publish_return_click(object sender, EventArgs e)
    {
        try
        {
            ...
            string extenedBody = txtContent_extend.Value;
            string postBody = txtContent.Value;
