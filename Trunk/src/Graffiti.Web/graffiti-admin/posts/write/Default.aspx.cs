...
public partial class graffiti_admin_posts_write_Default : ControlPanelPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Category uncategorized = _categoryService.FetchCachedCategory(_categoryService.UncategorizedName(), false);
        Post post = null;

        if (Request.QueryString["id"] != null)
            post = _postService.FetchPost(Request.QueryString["id"]);

        ProcessCategoryDropdownList(isAdmin, uncategorized);
        ...
        if(!IsPostBack)
        {
            ...
            {
                post =  _postService.FetchPost(Request.QueryString["nid"]);
                if (post.IsLoaded)
                {
                    if (isAdmin)
                    {
            ...
            if(post != null)
            {
                ...
                VersionStoreCollection vsc = new VersionStoreCollection(_versionStoreService.FetchVersionHistory(post.Id));
                    ...
                    if(versionToEdit > -1)
                    {
                        foreach(Post px in the_Posts)
                        {
                            if (px.Version == versionToEdit)
                            {
                                ...
                                CategoryCollection cats = new CategoryCollection(_categoryService.FetchCachedCategories());
            ...
            {
                ListItem liUncat = CategoryList.Items.FindByText(_categoryService.UncategorizedName());
                if (liUncat != null)
    ...
    private void SetDefaultFormValues(bool isAdmin)
    {
        ...
        if (isAdmin || _rolePermissionService.GetPermissions(Int32.Parse(CategoryList.SelectedValue), GraffitiUsers.Current).Publish)
    ...
    private void ProcessCategoryDropdownList(bool isAdmin, Category uncategorized)
    {
        if (!IsPostBack || Request.Form[CategoryList.UniqueID] != CategoryList.SelectedValue)
        {
            ...
            {
                if(_rolePermissionService.GetPermissions(parent.Id, GraffitiUsers.Current).Edit)
                ...
                {
                    if(_rolePermissionService.GetPermissions(child.Id, GraffitiUsers.Current).Edit)
                        CategoryList.Items.Add(new ListItem("--" + Server.HtmlDecode( child.Name ), child.Id.ToString()));
                }
            }

            if (_rolePermissionService.GetPermissions(uncategorized.Id, GraffitiUsers.Current).Edit)
        ...
        try
        {
            ...
            if (catItem.Value == "-1")
            {
                try
                {
                    ...
                    temp = _categoryService.SaveCategory(temp);
            ...
            {
                c = _categoryService.FetchCachedCategory(Int32.Parse(catItem.Value), false);
            }

            string pid = Request.QueryString["id"];
            Post p = pid == null ? new Post() : _postService.FetchPost(pid);
    ...
    {
        PostCollection pc = new PostCollection(_postService.FetchPosts().Where(x => x.CategoryId == p.CategoryId)
            .Where(x => x.Name == Util.CleanForUrl(!string.IsNullOrEmpty(p.Name) ? p.Name : p.Title)).ToList());
        if (!p.IsNew)
            pc = new PostCollection(pc.Where(x => x.Id != p.Id).ToList());
