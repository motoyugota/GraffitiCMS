var TEDragTableHandler =
{
	...
	{
		var myCoords = TETools.GetWindowPosition( w, domNode ) ;
		var xMin = myCoords.x ;
		var yMin = myCoords.y ;
	...
	"_GetBorderCells" : function( w, tableNode, tableMap, mouse )
	{
		...
		{
			var pos = TETools.GetWindowPosition( w, cells[i] ) ;
		...
		/*
		var rowNode = TETools.GetElementAscensor( rbCell, "tr" ) ;
		...
		*/
		...
		var colIdx = TETableHandler._GetCellIndexSpan( tableMap, rowIdx, rbCell ) ;
		...
		lxDist = mouse.x - TETools.GetWindowPosition( w, lbCell ).x ;
	...
	{
		var row = TETools.GetElementAscensor( this._RightCell, "tr" ) ;
		return TETableHandler._GetCellIndexSpan( this._TableMap, row.rowIndex, this._RightCell ) ;
	},
	"_ResizeBarMouseDownListener" : function( evt )
	{
		if ( ! evt )
			evt = window.event ;
		if ( TEDragTableHandler._LeftCell )
			TEDragTableHandler._MouseMoveMode = 1 ;
		if ( TEBrowserInfo.IsIE )
			TEDragTableHandler._ResizeBar.filters.item("DXImageTransform.Microsoft.Alpha").opacity = 50 ;
		else
			TEDragTableHandler._ResizeBar.style.opacity = 0.5 ;
		TEDragTableHandler._OriginalX = evt.clientX ;

		// Calculate maximum and minimum x-coordinate delta.
		var borderIndex = TEDragTableHandler._GetResizeBarPosition() ;
		var offset = TETools.GetDocumentPosition( window, TE.EditingArea.IFrame ) ;
		var table = TETools.GetElementAscensor( TEDragTableHandler._LeftCell, "table" );
		var minX = null ;
		var maxX = null ;
		for ( var r = 0 ; r < TEDragTableHandler._TableMap.length ; r++ )
		{
			var leftCell = TEDragTableHandler._TableMap[r][borderIndex - 1] ;
			var rightCell = TEDragTableHandler._TableMap[r][borderIndex] ;
			var leftPosition = TETools.GetWindowPosition( TE.EditorWindow, leftCell ) ;
			var rightPosition = TETools.GetWindowPosition( TE.EditorWindow, rightCell ) ;
			var leftPadding = TEDragTableHandler._GetCellPadding( table, leftCell ) ;
			var rightPadding = TEDragTableHandler._GetCellPadding( table, rightCell ) ;
		...
		TEDragTableHandler._MinimumX = minX + offset.x ;
		TEDragTableHandler._MaximumX = maxX + offset.x ;
		TEDragTableHandler._LastX = null ;
	},
	"_ResizeBarMouseUpListener" : function( evt )
	{
		if ( ! evt )
			evt = window.event ;
		TEDragTableHandler._MouseMoveMode = 0 ;
		TEDragTableHandler._HideResizeBar() ;

		if ( TEDragTableHandler._LastX == null )
			return ;

		// Calculate the delta value.
		var deltaX = TEDragTableHandler._LastX - TEDragTableHandler._OriginalX ;
		...
		var table = TETools.GetElementAscensor( TEDragTableHandler._LeftCell, "table" ) ;
		var colArray = [] ;
		var tableMap = TEDragTableHandler._TableMap ;
		for ( var i = 0 ; i < tableMap.length ; i++ )
		{
			for ( var j = 0 ; j < tableMap[i].length ; j++ )
			{
				var cell = tableMap[i][j] ;
				var width = TEDragTableHandler._GetCellWidth( table, cell ) ;
		...
		colIndex = TEDragTableHandler._GetResizeBarPosition() ;
	...
	"_ResizeBarMouseMoveListener" : function( evt )
	{
		if ( ! evt )
			evt = window.event ;
		if ( TEDragTableHandler._MouseMoveMode == 0 )
			return TEDragTableHandler._MouseFindHandler( TE, evt ) ;
		else
			return TEDragTableHandler._MouseDragHandler( TE, evt ) ;
	},
	...
	"MouseMoveListener" : function( TE, evt )
	{
		if ( TEDragTableHandler._MouseMoveMode == 0 )
			return TEDragTableHandler._MouseFindHandler( TE, evt ) ;
		else
	...
	"_MouseFindHandler" : function( TE, evt )
	{
		if ( TE.MouseDownFlag )
			return ;
		...
		// Since this function might be called from the editing area iframe or the outer Telligent_Editor iframe,
		...
		{
			var leftPos = TETools.GetWindowPosition( TE.EditorWindow, this._LeftCell ) ;
			var rightPos = TETools.GetWindowPosition( TE.EditorWindow, this._RightCell ) ;
			...
			if ( inRangeFlag )
			{
				this._ShowResizeBar( TE.EditorWindow,
					TETools.GetElementAscensor( this._LeftCell, "table" ),
					{ "x" : mouseX, "y" : mouseY } ) ;
				return ;
			}
		}
		...
		node = TETools.GetElementAscensor( node, "table" ) ;
		var tableMap = TETableHandler._CreateTableMap( node ) ;
		var cellTuple = this._GetBorderCells( TE.EditorWindow, node, tableMap, { "x" : mouseX, "y" : mouseY } ) ;
	...
	"_MouseDragHandler" : function( TE, evt )
	{
		var mouse = { "x" : evt.clientX, "y" : evt.clientY } ;

		// Convert mouse coordinates in reference to the outer iframe.
		var node = evt.srcElement || evt.target ;
		if ( node.ownerDocument == TE.EditorDocument )
		{
			var offset = TETools.GetDocumentPosition( window, TE.EditingArea.IFrame ) ;
			mouse.x += offset.x ;
			mouse.y += offset.y ;
		...
		var docX = mouse.x + TETools.GetScrollPosition( window ).X ;
	...
	"_ShowResizeBar" : function( w, table, mouse )
	{
		if ( this._ResizeBar == null )
		{
			...
			TEDomTools.SetElementStyles( paddingBar, paddingStyles ) ;
			document.body.appendChild( paddingBar ) ;
			TETools.AddEventListener( paddingBar, "mousemove", this._ResizeBarMouseMoveListener ) ;
			TETools.AddEventListener( paddingBar, "mousedown", this._ResizeBarMouseDownListener ) ;
			TETools.AddEventListener( document, "mouseup", this._ResizeBarMouseUpListener ) ;
			TETools.AddEventListener( TE.EditorDocument, "mouseup", this._ResizeBarMouseUpListener ) ;
			...
			filler.border = 0 ;
			filler.src = TEConfig.BasePath + "images/spacer.gif" ;
			...
			TETools.AddEventListener( filler, "dragstart", disabledListener ) ;
			TETools.AddEventListener( filler, "selectstart", disabledListener ) ;
		}

		var paddingBar = this._ResizeBar ;
		var offset = TETools.GetDocumentPosition( window, TE.EditingArea.IFrame ) ;
		var tablePos = TETools.GetWindowPosition( w, table ) ;
		...
		var paddingStyles =
		{
			...
			'height'	: barHeight + 'px',
			'width'		: barWidth + 'px',
			'left'		: ( offset.x + mouse.x + TETools.GetScrollPosition( w ).X - barWidth / 2 ) + 'px'
		} ;
		if ( TEBrowserInfo.IsIE )
		...
		TEDomTools.SetElementStyles( filler,
			{
		...
		TEDomTools.SetElementStyles( visibleBar,
			{
				position		: 'absolute',
	...
	"_HideResizeBar" : function()
	{
		if ( this._ResizeBar )
			...
			TEDomTools.SetElementStyles( this._ResizeBar,
				{
					top		: '-100000px',
					left	: '-100000px'
				} ) ;
...
TE.Events.AttachEvent( "OnMouseMove", TEDragTableHandler.MouseMoveListener ) ;
